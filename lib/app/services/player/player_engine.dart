...
/// - [MediaKitEngine]：libmpv + FFmpeg，处理 FLAC 32KB 帧缓冲上限、
///   DSF/DSD/APE/WMA 等异形编码。
///
/// [FnPlayerService] 持有唯一逻辑队列，把队列交给当前曲目所属引擎播放；
/// 引擎切换只在歌曲边界发生。
///
...
/// 引擎无关的可播放源：在线流或本地缓存文件。
///
/// [StreamCacheService.sourceFor] 命中缓存时返回本地文件路径（无需鉴权头），
/// 未命中返回在线流地址 + 鉴权头。两个引擎据此各自构建原生源。
class PlayerSource {
  final String uri;
