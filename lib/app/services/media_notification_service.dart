...
class MediaNotificationService {
  ...
  static Future<void> init({bool force = false}) async {
    ...
    final snap = AppPlayerState.instance.snapshot.value;
    if (!force && snap.song == null && !snap.isPlaying) {
      ...
      if (_initListener == null) {
        _initListener = () {
          final current = AppPlayerState.instance.snapshot.value;
          if (current.song == null && !current.isPlaying) return;
          if (_initListener != null) {
            AppPlayerState.instance.snapshot.removeListener(_initListener!);
            _initListener = null;
          }
          init(force: true);
        };
        AppPlayerState.instance.snapshot.addListener(_initListener!);
      }
      return;
    }
    await _initHandler();
  ...
  static Future<void> _initHandler() async {
    ...
    try {
      _audioHandler = await AudioService.init(
        builder: () => _FnMusicAudioHandler(FnPlayerService.instance),
        config: const AudioServiceConfig(
          androidNotificationChannelId: 'com.fnmusic.app.playback',
...
class _FnMusicAudioHandler extends BaseAudioHandler
  ...
  static const String _carDrawableUriPrefix =
      'android.resource://${ApiClient.kApplicationId}/drawable/';
  static const String _defaultAlbumArtworkName = 'ic_car_album';

  _FnMusicAudioHandler(this.player) {
    AppPlayerState.instance.snapshot.addListener(_syncFromPlayer);
  ...
  Future<void> _loadPlatformCapabilities() async {
    ...
    _syncPlaybackState(AppPlayerState.instance.snapshot.value);
  }
  ...
  Future<Uri?> _getLocalCoverUri(String coverId) async {
    final url = ApiClient.instance.coverUrl(
      coverId,
    ...
    try {
      ...
      final cacheFile = await _coverCache.getSingleFile(
        url,
        headers: ApiClient.imageAuthHeaders(),
      );
    ...
    try {
      ...
      final filePath = '$dir/${coverId}_512.jpg';
  ...
  MediaItem _itemFromSong(SongEntity song, {bool current = false}) {
    ...
    final artistText = (song.artistDisplay ?? '').trim();
    final songAndArtist = artistText.isEmpty
        ? titleText
        : '$titleText · $artistText';
    final albumName = song.albumDisplay;
    ...
    if (song.coverId != null && song.coverId!.isNotEmpty) {
      if (current &&
        ...
        final remote = ApiClient.instance.coverUrl(
          song.coverId!,
          size: _systemCoverSize,
        );
        if (remote != null) artUri = Uri.tryParse(remote);
      }
    }
    ...
    final isCurrentSong =
        AppPlayerState.instance.snapshot.value.song?.guid == song.guid;
    ...
    if (lyricOnTop && lyricLine != null) {
      return MediaItem(
        ...
        artHeaders: ApiClient.imageAuthHeaders(),
        extras: carLyricsExtras,
      );
    }
    final effectiveArtist = lyricLine ?? song.artistDisplay;
    return MediaItem(
      id: song.guid,
      ...
      displayDescription: lyricLine != null ? song.artistDisplay : null,
  ...
  Future<MediaItem?> getMediaItem(String mediaId) async {
    ...
    for (final song in AppPlayerState.instance.snapshot.value.queue) {
      if (song.guid == mediaId) return _itemFromSong(song);
    }
    return null;
  }
  ...
  Future<List<SongEntity>> _loadHistorySongs() async {
    ...
    final tracks = await ApiClient.instance.getPlayHistory(page: 1, size: 40);
    return tracks.map(SongEntity.fromTrack).toList();
  }

  Future<List<SongEntity>> _loadFavoriteSongs() async {
    await _ensureApiAuth();
    final tracks = await FeiNiuFavoriteService.instance.fetchFavoriteTracks();
    return tracks.map(SongEntity.fromTrack).toList();
  }

  Future<List<SongEntity>> _loadTracks() async {
    await _ensureApiAuth();
    final pageData = await FnTrackService.instance.fetchTracks(page: 1);
    return pageData.list.map(SongEntity.fromTrack).toList();
  }

  Future<List<SongEntity>> _loadArtistSongs(String artistId) async {
    await _ensureApiAuth();
    final tracks =
        await FnArtistService.instance.fetchArtistTracks(artistId);
    return tracks.map(SongEntity.fromTrack).toList();
  }

  Future<List<SongEntity>> _loadAlbumSongs(String albumId) async {
    await _ensureApiAuth();
    final tracks = await FnAlbumService.instance.fetchAlbumTracks(albumId);
    return tracks.map(SongEntity.fromTrack).toList();
  }

  Future<List<SongEntity>> _loadGenreSongs(String genreId) async {
    await _ensureApiAuth();
    final tracks = await FnGenreService.instance.fetchGenreTracks(genreId);
    return tracks.map(SongEntity.fromTrack).toList();
  }
  ...
  Future<List<MediaItem>> _loadArtists() async {
    try {
      await _ensureApiAuth();
      final pageData = await FnArtistService.instance.fetchArtists(page: 1);
  ...
  Future<List<MediaItem>> _loadAlbums() async {
    try {
      await _ensureApiAuth();
      final pageData = await FnAlbumService.instance.fetchAlbums(page: 1);
  ...
  Future<List<MediaItem>> _loadGenres() async {
    try {
      await _ensureApiAuth();
      final genres = await FnGenreService.instance.fetchGenres();
      return genres.map(_genreItem).toList();
    } catch (error) {
  ...
  Future<MediaItem> _artistItem(FnArtist artist) async {
  ...
  Future<MediaItem> _albumItem(FnAlbum album) async {
    return MediaItem(
  ...
  MediaItem _genreItem(FnGenre genre) {
    return MediaItem(
  ...
  Future<void> _ensureApiAuth() {
    return _apiAuthReady ??= AuthService.instance.initialize().then(
      (_) {},
    );
  }
  ...
  List<MediaItem> _cacheBrowseSongs({
    ...
    final queue = List<SongEntity>.unmodifiable(
      songs.where((song) => song.guid.isNotEmpty),
    );
    return List<MediaItem>.generate(queue.length, (index) {
      final song = queue[index];
      final mediaId = '$parentMediaId:track:$index:${song.guid}';
  ...
  MediaItem _browseSongItem(SongEntity song, String mediaId) {
    final artist = (song.artistDisplay ?? '').trim();
    return MediaItem(
      id: mediaId,
      title: song.title,
      ...
      playable: true,
      displayTitle: song.title,
      displaySubtitle: artist.isEmpty ? song.albumDisplay : artist,
      extras: const <String, dynamic>{
        AndroidContentStyle.playableHintKey:
            AndroidContentStyle.listItemHintValue,
      },
  ...
  Future<void> _playBrowseMediaId(String mediaId) async {
    if (mediaId == _browseRoamStartId) {
      await player.startRoam();
      return;
    }
    final browseSong = _browseSongs[mediaId];
    if (browseSong != null) {
      await player.setQueue(browseSong.queue, index: browseSong.index);
      return;
    }
    final queue = AppPlayerState.instance.snapshot.value.queue;
    final index = queue.indexWhere((song) => song.guid == mediaId);
    if (index >= 0) await player.setQueue(queue, index: index);
  }
  ...
  PlaybackState _stateFromSnap(PlayerSnapshot snap) {
    final playing = snap.isPlaying;
    final queueIndex = _publishedQueueIds.indexOf(snap.song?.guid ?? '');
    final showClose =
    ...
    return PlaybackState(
      ...
      speed: AppPlayerState.instance.speed.value,
  ...
  // 就弹出系统权限对话框。
  void _requestNotificationPermissionIfNeeded(PlayerSnapshot snap) {
  ...
  void _prewarmQueueCovers(PlayerSnapshot snap) {
  ...
  Future<void> _publishWithLocalArt(SongEntity song) async {
    ...
    if (song.guid != _lastSongId) return; // 已切歌，丢弃过期结果
    ...
    _syncQueue(AppPlayerState.instance.snapshot.value);
    _syncMediaItem();
    if (!hasLocal) {
  ...
  Future<void> _syncAndUpdateCover(SongEntity song) async {
    ...
    try {
      ...
      if (resolved.path != null && song.guid == _lastSongId) {
  ...
  /// 将应用内播放队列发布给系统媒体会话。
  void _syncQueue(PlayerSnapshot snap) {
    ...
    _publishedQueueIds = sessionQueue.map((song) => song.guid).toList();
    queue.add(items);
  }

  List<SongEntity> _queueForMediaSession(PlayerSnapshot snap) {
    if (!player.isRoaming || snap.queue.isEmpty) {
      _roamQueueAnchorId = null;
      _stableRoamQueue = null;
      return snap.queue;
    }
    final anchorId = snap.queue.first.guid;
  ...
  void _syncMediaItem() {
    final current = AppPlayerState.instance.snapshot.value.song;
  ...
  void _syncPlaybackState(PlayerSnapshot snap) {
    ...
    final stateKey = [
      ...
      AppPlayerState.instance.speed.value,
      _isFavorite,
  ...
  void _onNotificationSettingsChanged() {
    ...
    _syncMediaItem();
    playbackState.add(_stateFromSnap(AppPlayerState.instance.snapshot.value));
  }

  void _refreshFavoriteState() {
    final song = AppPlayerState.instance.snapshot.value.song;
    if (song == null) return;
    // 从服务器查询收藏状态
    FeiNiuFavoriteService.instance.isFavorite(song.guid).then((fav) {
      _updateFavorite(fav);
    });
  }
  ...
  Future<void> customAction(String name, [Map<String, dynamic>? extras]) async {
    ...
    if (name == _actionFavorite) {
      ...
      if (_isFavorite) {
        ...
        try {
          await FeiNiuFavoriteService.instance.favorite(song.guid);
